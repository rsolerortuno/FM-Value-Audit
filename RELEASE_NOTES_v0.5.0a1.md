...
The repository now uses executable GitHub Actions controls rather than fixed test or coverage images.

- The full suite runs on Python 3.11, 3.12 and 3.13 for every push and pull request.
- Branch-aware coverage is calculated during the run and must remain at least 85%.
- JUnit XML, coverage JSON/XML, registry audit, temporal audit and validation summary are uploaded as commit-bound workflow artifacts.
- Ruff lint and formatting are blocking jobs.
- Wheel and source distributions are built and smoke-tested from the checked-out commit.
- Strict mypy is a blocking job and passes on the corrected source tree.
- The corrected source tree reports 70 passing tests, 86.78% branch-aware coverage, clean Ruff and strict mypy across 44 source files; live GitHub Actions remains authoritative for the current commit.
